...
func (l *Library) Pull(ctx context.Context, from, to, arch string) error {
	...
	dst, tmpName, err := func() (string, string, error) {
		...
		if err := os.Remove(tmpName); err != nil {
			return "", "", fmt.Errorf("unable to remove temporary file %s: %w", err)
		}
...
// copyImageToDest will take a tmp image, and copy it to
// final location, taking care to remove the overridden image
// if necessary.
func copyImageToDest(tmpImage, dest string) error {
	err := os.Remove(dest)
	if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("failed to remove overridden image: %s", err)
	}

	err = os.Rename(tmpImage, dest)
	if err != nil {
